...
object StoryNewsPrefs {
    ...
    private const val KEY_READER_PHOTO = "READER_PHOTO"
    ...
    fun saveReaderPhoto(context: Context, photoUrl: String) {
        val prefs = context.getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE)
        prefs.edit().putString(KEY_READER_PHOTO, photoUrl).apply()
    }

    fun getReaderPhoto(context: Context): String {
        val prefs = context.getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE)
        return prefs.getString(KEY_READER_PHOTO, "") ?: ""
    }
}
